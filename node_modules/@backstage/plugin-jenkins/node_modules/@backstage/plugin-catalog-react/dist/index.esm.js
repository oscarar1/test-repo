import { createApiRef, useApi, errorApiRef, storageApiRef } from '@backstage/core-plugin-api';
import React, { createContext, useEffect, useContext, useState, useCallback, forwardRef, useMemo, Fragment } from 'react';
import { Alert, Autocomplete } from '@material-ui/lab';
import { useNavigate, generatePath } from 'react-router';
import { useAsync, useAsyncFn, useDebounce, useObservable } from 'react-use';
import { createRouteRef, useRouteRefParams, useApi as useApi$1, Link, identityApiRef, OverflowTooltip, Table, alertApiRef, Select, configApiRef } from '@backstage/core';
import { ENTITY_DEFAULT_NAMESPACE, VIEW_URL_ANNOTATION, EDIT_URL_ANNOTATION, SOURCE_LOCATION_ANNOTATION, parseLocationReference, RELATION_MEMBER_OF, getEntityName, RELATION_OWNED_BY, serializeEntityRef, RELATION_PART_OF } from '@backstage/catalog-model';
import { compact, isEqual, groupBy, chunk, capitalize } from 'lodash';
import { Box, Typography, FormControlLabel, Checkbox, TextField, makeStyles, Card, List, MenuItem, ListItemIcon, ListItemText, ListItemSecondaryAction } from '@material-ui/core';
...
const EntityListProvider = ({
  ...
  const catalogApi = useApi$1(catalogApiRef);
...
class EntityOwnerFilter {
  ...
  filterEntity(entity) {
    return this.values.some((v) => getEntityRelations(entity, RELATION_OWNED_BY).some((o) => formatEntityRefTitle(o, {defaultKind: "group"}) === v));
  }
}
...
function useOwnUser() {
  const catalogApi = useApi$1(catalogApiRef);
  const identityApi = useApi$1(identityApiRef);
...
const EntityLifecyclePicker = () => {
  ...
  if (!availableLifecycles.length)
    return null;
  const onChange = (lifecycles) => {
    updateFilters({
      lifecycles: lifecycles.length ? new EntityLifecycleFilter(lifecycles) : void 0
    });
  };
  return /* @__PURE__ */ React.createElement(Box, {
    ...
    value: (_b = (_a = filters.lifecycles) == null ? void 0 : _a.values) != null ? _b : [],
...
const EntityOwnerPicker = () => {
  ...
  const availableOwners = useMemo(() => [
    ...new Set(backendEntities.flatMap((e) => getEntityRelations(e, RELATION_OWNED_BY).map((o) => formatEntityRefTitle(o, {defaultKind: "group"}))).filter(Boolean))
  ].sort(), [backendEntities]);
  if (!availableOwners.length)
    return null;
  const onChange = (owners) => {
    updateFilters({
      owners: owners.length ? new EntityOwnerFilter(owners) : void 0
    });
  };
  return /* @__PURE__ */ React.createElement(Box, {
    pb: 1,
    ...
    value: (_b = (_a = filters.owners) == null ? void 0 : _a.values) != null ? _b : [],
    onChange: (_, value) => onChange(value),
    renderOption: (option, {selected}) => /* @__PURE__ */ React.createElement(FormControlLabel, {
      control: /* @__PURE__ */ React.createElement(Checkbox, {
        icon: icon$1,
...
const EntityTypePicker = () => {
  const alertApi = useApi$1(alertApiRef);
